...
fn generate_undo_command(command_pointer: i32, command_stack: &Vec<Box<dyn Command>>) -> Option<Box<dyn Command>> {
  ...
  let command = command_stack.iter()
    .enumerate()
    .rev()
...
pub fn generate_time_travel_command(command_type: &CommandType, command_pointer: i32, command_stack: &Vec<Box<dyn Command>>) -> Option<Box<dyn Command>> {
  match command_type {
